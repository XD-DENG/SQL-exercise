#挑战，仅通过diagrams写出sql查询语句
-- 8.1
use sql_exercise;
-- Obtain the names of all physicians that have performed a medical procedure they have never been certified to perform.
-- 还未被认证，已经在进行疗程的医师名字。

select phy.Name from Prescribes p join Physician phy on p.Physician = phy.EmployeeID where phy.EmployeeID not in (select Physician from Trained_In);

-- 8.2
-- Same as the previous query, but include the following information in the results: Physician name, name of procedure, date when the procedure was carried out(进行), name of the patient the procedure was carried out on（进行上）.
